...
import {
  ...
  riderPackingCategories,
  volunteerPackingCategories,
  type PackingItem,
} from "@/lib/mock-data";
...
function PackingPage() {
  ...
  const [items, setItems] = useState<PackingItem[]>(() => getDefaultPackingList("rider"));
  const [showIncompleteOnly, setShowIncompleteOnly] = useState(false);
  const [categoryFilter, setCategoryFilter] = useState<string>("all");
  const [newItem, setNewItem] = useState("");
  const [newCategory, setNewCategory] = useState<string>("");

  const categories = preset === "rider" ? riderPackingCategories : volunteerPackingCategories;

  const switchPreset = (p: "rider" | "volunteer") => {
    setPreset(p);
    setItems(getDefaultPackingList(p));
  ...
  const addCustom = () => {
    ...
    setItems((prev) => [
      ...prev,
      { id: `c-${crypto.randomUUID()}`, label, category: cat, checked: false, custom: true },
    ]);
    setNewItem("");
  };

  const reset = () => setItems(getDefaultPackingList(preset));
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground max-w-xl">
            Toggle items as you pack. Add your own. State stays in this session — sample content only.
          </p>
        </div>
        <Tabs value={preset} onValueChange={(v) => switchPreset(v as "rider" | "volunteer")}>
          <TabsList>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="min-w-[220px] flex-1">
              ...
              <Progress
                ...
                aria-label={`Packing completion ${pct} percent`}
              />
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <label className="flex items-center gap-2 text-sm">
                <Checkbox
                  ...
                  onCheckedChange={(v) => setShowIncompleteOnly(!!v)}
